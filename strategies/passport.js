...
module.exports = function(passport) {
  ...
  // logged in or not)
      ...
      function(req, email, password, done) {
        ...
        process.nextTick(function() {
          if(!req.user){
            User.registerNewUser(email, password, function(err, userCreated){
              if(err){                
                return done(null, false, req.flash('Signup Error', 'Sign up failed - ' + err.message));
              }              
              return done(null, userCreated);
            });
          } else if(!req.user.email) {
            //...presumably they're trying to connect a local account
            User.registerExistingUser(email, password, function(err, userCreated){
              if(err){                
                return done(null, false, req.flash('Signup Error', 'Sign up failed - ' + err.message));
              }
              return done(null, userCreated);
  ...
  // logged in or not)
    ...
    process.nextTick(function() {
      ...
      if (!req.user) {
          ...
          'google.id' : profile.id
            ...
            function(err, user) {
              ...
              if (user) {
                ...
                // if there is a user id already but no token (user was linked
                // at one point and then removed)
        ...
        user.google.token = token;
        user.google.name = profile.displayName;
        user.google.email = (profile.emails[0].value || '').toLowerCase(); // pull
                                                                            // the
                                                                            // first
                                                                            // email
